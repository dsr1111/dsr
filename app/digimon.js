...
async function fetchCSV() {
  const [skill1Data, skill2Data, skill3Data] = await Promise.all([
    loadCSV("../data/csv/skill1.csv"),
    loadCSV("../data/csv/skill2.csv"),
    loadCSV("../data/csv/skill3.csv"),
  ]);
  ...
  rows.forEach((row) => {
    ...
    const skillHtml = (skill, skillNumber, digimonName) => {
      ...
      const effectDescriptions = {
        ...
        회복: "* 회복",
        혼란: "* 혼란<br>공격 시 29.5% 확률로 발생됩니다.<br>일정 턴 동안 명령이 불가능해집니다.<br>피아식별 없이 행동하게 됩니다.",
      };
