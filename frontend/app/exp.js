...
let initialExp = 0;
// 진화 상태 저장
let isMatureEvolution = false;
let isPerfectEvolution = false;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('current-level').addEventListener('input', function () {
        ...
        if (initialLevel >= 1 && initialLevel <= 15) {
            ...
            isMatureEvolution = document.getElementById('evolution-checkbox').checked;
            isPerfectEvolution = false;
        } else if (initialLevel >= 50 && initialLevel <= 60) {
            checkboxContainer.classList.remove('hidden');
            checkboxText.textContent = '완전체 진화 여부';
            isPerfectEvolution = document.getElementById('evolution-checkbox').checked;
            isMatureEvolution = false;
        } else {
    ...
    // 체크박스 변경 이벤트 리스너 추가
    document.getElementById('evolution-checkbox').addEventListener('change', function () {
        if (initialLevel >= 1 && initialLevel <= 15) {
            isMatureEvolution = this.checked;
            isPerfectEvolution = false;
        } else if (initialLevel >= 50 && initialLevel <= 60) {
            isPerfectEvolution = this.checked;
            isMatureEvolution = false;
        }
        updateResults();
    });
    ...
    // 초기 계산
    // calculateNeededExp function seems unused in the original script but was called in DOMContentLoaded.
    // It refers to 'target-level' which element does NOT exist in the provided HTML.
    // The original script has logic for "target level" calculation commented out or partially removed.
    // I will keep the call if it works, or comment it out if it fails.
    // Wait, looking at lines 533-582 in original file, calculateNeededExp uses 'target-level'.
    // But 'target-level' input is NOT in the HTML structure I saw.
    // The HTML only has 'current-level', 'current-exp', 'evolution-checkbox'.
    // It seems the original code had leftover functions.
    // I will include the function for completeness but wrap the initial call in try-catch or check existence.

    // Actually, looking at the code, it seems the user might have removed the target level feature but kept the code.
    // I will copy the code as is.
    try {
        calculateNeededExp();
    } catch (e) {
        // ignore error if elements missing
    }
...
function calculateNeededExp() {
    ...
    const targetLevelInput = document.getElementById('target-level'); // Might be missing

    if (!targetLevelInput) return; // Exit if element doesn't exist
    ...
    // 결과 표시
    const totalExpNeededEl = document.getElementById('total-exp-needed');
    if (totalExpNeededEl) totalExpNeededEl.textContent = totalExpNeeded.toLocaleString();

    // 레벨 시뮬레이션 초기값 설정
    const simCurrentLevelEl = document.getElementById('sim-current-level');
    if (simCurrentLevelEl) simCurrentLevelEl.textContent = currentLevel;

    const simCurrentExpEl = document.getElementById('sim-current-exp');
    if (simCurrentExpEl) simCurrentExpEl.textContent = currentExp.toLocaleString();

    const simLevelExpEl = document.getElementById('sim-level-exp');
    if (simLevelExpEl) simLevelExpEl.textContent = expTable[currentLevel - 1].exp.toLocaleString();
        ...
        for (let i = 0; i < count; i++) {
            ...
            while (nextLevel < 100) {
                ...
                if (nextExp >= expToNext) {
                    nextExp -= expToNext;
                    nextLevel++;
                    // 진화 레벨에서 경험치 초기화 로직 수정
                    if (evolutionLevels.includes(nextLevel)) {
                        if (nextLevel === 15 && !isMatureEvolution) {
                            nextExp = 0;
                            break;
                        } else if (nextLevel === 60 && !isPerfectEvolution) {
                            nextExp = 0;
                            break;
...
function updateResultTable(potionCounts) {
    ...
    for (const potion of sortedPotions) {
        ...
        row.innerHTML = `
            <td class="potion-name"><img loading="lazy" src="https://media.dsrwiki.com/dsrwiki/item/${imageFileName}" alt="${potion} EXP 물약" class="potion-img">${potion} EXP 물약</td>
            <td>${count}</td>
